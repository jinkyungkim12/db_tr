...
-- union: 2개의 테이블을 하나의 로우에 표현할 때 
-- 최소 2개 이상의 테이블/ 컬럼의 데이터 타입, 컬럼 갯수가 똑같아야 함
-- 공통항목이 없을 떄 -> 데이터 as height 이렇게 표현하기: 데이터가 공백이어도 오케이 그러나 갯수는 늘 맞춰야 한다.

-- union all : 중복 데이터 거르지 않고, raw data 다 뽑아오기 때문에 속도가 더 빠름
-- union: 중복된 것 걸러냄
